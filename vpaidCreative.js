var debugVpaid = (function () {
  try {
    return localStorage.getItem("oaplayervpaiddebug") === "true";
  } catch (e) {
    return false;
  }
})();
...
VPAIDCreative.prototype.createSkipButton_ = function () {
  ...
  this.skipButton_.style.cssText =
    " position: absolute; bottom: 5%;right: 2%;padding: 1% 3%;background-color: rgba(0, 0, 0, 0.5);color: white;border: 2px solid white;border-radius: 5px;font-size: clamp(18px, 2.5vw, 24px);cursor: not-allowed;z-index: 1000;transition: all 0.3s ease;font-family: Arial, sans-serif;";
...
VPAIDCreative.prototype.updateSkipButtonText_ = function (secondsRemaining) {
  if (this.skipButton_) {
    if (secondsRemaining > 0) {
      this.skipButton_.textContent = this.textSkipIn_.replace("[number]", secondsRemaining);
    } else {
      this.updateShieldSize();
      this.skipButton_.textContent = this.textSkip_;
    }
  }
};
